...
MODELO
ggplot(ACRES.FLORA, aes(x=Acres, y=n)) + geom_point() + ggtitle("Diagrama de Dispersion") + xlab("Hectareas") + ylab("Riqueza de Especies") 
CORR.FLORA <- round(cor(ACRES.FLORA[,3:4]),2)
...
CORR.FAUNA <- round(cor(ACRES.FAUNA[,3:4]),2)
...
CORR.FUNGI <- round(cor(ACRES.FUNGI[,3:4]),2)
...
species.wrst <- filter(species, `Park Name`=="Wrangell - St Elias National Park and Preserve")
species.summary.wrst <- select(species.wrst, `Park Name`, `Category`, `Family`, `Scientific Name`, `Nativeness`)
...
CORRELACIÓN Y GRAFICOS DE DISPERSIÓN
